...
class EnrollDB:
    ...
    def listDeptProfessors(self, deptName):
        ...
        cursor = self.cnx.cursor()
        query = "SELECT pname FROM prof WHERE dname ="+deptName+";"
        cursor.execute(query)
    ...
    def listCourseStudents(self, courseNum):
        ...
        cursor = self.cnx.cursor()
        query = "SELECT sid, sname, cnum, secnum FROM student natural join enroll where cnum = " + str(courseNum)+";"
        cursor.execute(query)
        for(sid, sname, cnum, secnum) in cursor:
            output += "\n"+sid+", "+sname+", "+str(cnum)+", "+str(secnum)
        return output
    ...
    def computeGPA(self, studentId):
        ...
        cursor = self.cnx.cursor()
        query = "SELECT student.sid, avg(grade) as gpa \
            FROM enroll right join student on enroll.sid = student.sid where student.sid ="+ str(studentId) +"group by sid;"
        cursor.execute(query)
        return cursor
    ...
    def addStudent(self, studentId, studentName, sex, birthDate):
        ...
        cursor = self.cnx.cursor()
        query = "INSERT INTO student values(sid="+studentId+",sname ="+studentName+", sex ="+sex+",birthdate ="+birthDate+");"
        cursor.execute(query)
        return
    ...
    def deleteStudent(self, studentId):
        ...
        # TODO: Execute statement. Make sure to commit
        cursor = self.cnx.cursor()
        query = "DELETE FROM student WHERE sid = "+str(studentId)+";"
        cursor.execute(query)
        return
    ...
    def updateStudent(self, studentId, studentName, sex, birthDate, gpa):
        ...
        # TODO: Execute statement. Make sure to commit
        cursor = self.cnx.cursor()
        query = "UPDATE student SET sname ="+studentName+", sex="+sex+",birthdate="+birthDate+",gpa="+gpa+ "WHERE sid="+studentId+";"
        cursor.execute(query)
        return
    ...
    def newEnroll(self, studentId, courseNum, sectionNum, grade):
        ...
        # TODO: Execute statement. Make sure to commit
        cursor = self.cnx.cursor()
        query = "INSERT enroll VALUES(sid="+studentId+",cnum="+courseNum+", secnum="+sectionNum+",grade = "+grade+");"
        cursor.execute(query)
        return
    ...
    def updateStudentGPA(self, studentId):
        ...
        # TODO: Execute statement. Make sure to commit
        cursor = self.cnx.cursor()
        query = "UPDATE student SET gpa = (SELECT avg(grade) from enroll where sid="+studentId+") WHERE sid="+studentID+";"
        cursor.execute(query)
        return
    ...
    def removeStudentFromSection(self, studentId, courseNum, sectionNum):
        ...
        # TODO: Execute statement. Make sure to commit
        cursor = self.cnx.cursor()
        query = "DELETE FROM enroll WHERE sid ="+studentId+"AND"+"cnum ="+courseNum+"AND"+"secnum="+sectionNum+";"
        cursor.execute(query)
        updateStudentGPA(self, studentID)
        return
    ...
    def updateStudentMark(self, studentId, courseNum, sectionNum, grade):
        ...
        # TODO: Execute statement. Make sure to commit
        cursor = self.cnx.cursor()
        query = "UPDATE enroll SET grade ="+grade+"WHERE sid="+studentID+"AND cnum="+courseNum+"AND secnum="+sectionNum+";"
        cursor.execute(query)
        updateStudentGPA(self,studentId)
        return
    ...
    def query1(self):
        ...
        # TODO: Execute the query and return a cursor
        cursor = self.cnx.cursor()
        query = "SELECT student.sid, student.sname FROM student LEFT JOIN enroll ON stduent.sid = enroll.sid WHERE enroll.sid is null;"
        cursor.execute(query)
        return cursor
    ...
    def query2(self):
        ...
        Order by GPA descending then student name ascending and show only the top 5."""
         ...
         # TODO: Execute the query and return a cursor
        cursor = self.cnx.cursor()
        query = "select student.sid, student.sname,count(distinct (secnum)) as numcourses, avg(grade) as gpa from enroll right join student on enroll.sid = student.sid where birthdate > date '1992-03-15' group by sid having numcourses=0 or avg(grade) >3.1 order by gpa desc limit 5;"
        cursor.execute(query)
        return cursor
        return None

    def query3(self):
        """For each course, return the number of sections (numsections), total number of students enrolled (numstudents),
        average grade (avggrade), and number of distinct professors who taught the course (numprofs).
    ...
    def query4(self):
        ...
        # TODO: Execute the query and return a cursor
        cursor = self.cnx.cursor()
        query = "SELECT sname where gpa > cnum AND secnum average gpa"
        cursor.execute(query)
        return None
